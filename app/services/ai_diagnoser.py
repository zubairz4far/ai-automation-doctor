            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "Disambiguation rules: choose configuration rather than data_mapping when the problem is the "
                        "selected node mode/action/resource/request contract itself, even if the message mentions a body "
                        "shape or data format. Choose data_mapping when valid node configuration receives runtime data "
                        "whose fields, types, records, selectors, expressions, or items do not match what the node expects. "
                        "Choose webhook rather than network when the callback listener, route, registration, or live "
                        "deployment endpoint is absent; network requires explicit transport/connectivity evidence such as "
                        "DNS, TCP, socket, TLS, connection, or transport failure. Opaque vendor/internal/policy codes with "
                        "no evidence for another class are unknown, not network merely because a remote system returned them. "
                        "Prefer the most specific supported class, but do not invent evidence to avoid unknown. "
