...
class PatchValidator:
    ...
    def validate(self, proposal: PatchProposal) -> None:
        ...
        for operation in proposal.operations:
            ...
            _, root, node_segment, field_name = parts
            if root != "nodes" or not node_segment:
                raise PatchValidationError(f"Unsupported mutation path: {operation.path}")
            if field_name not in ALLOWED_NODE_FIELDS:
                raise PatchValidationError(f"Forbidden mutation path: {operation.path}")
            self._validate_value(field_name, operation.value)
    ...
    def _validate_value(field_name: str, value: object) -> None:
        ...
        if field_name == "maxTries" and not 1 <= value <= 5:
            raise PatchValidationError("maxTries must be between 1 and 5.")
        if field_name == "waitBetweenTries" and not 250 <= value <= 60_000:
